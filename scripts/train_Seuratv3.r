...
args$ckpt_dir <- file.path(args$ckpt_dir, sprintf("%s_Seuratv3_%d_%s", dataset_str, args$subset_genes, strftime(Sys.time(),"%m_%d-%H_%M_%S")))
...
fpath <- file.path(args$ckpt_dir, sprintf("%s_Seuratv3.h5seurat", dataset_str))
...
if (!args$no_eval) {
    ...
    fpath <- file.path(args$ckpt_dir, sprintf("%s_Seuratv3.h5ad", dataset_str))
